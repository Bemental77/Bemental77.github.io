...
static void reios_boot()
{
	...
	if (extension == "elf")
	{
		...
		if (settings.platform.isConsole())
		{
			...
			// One-shot snapshot at IPL entry, parity-test 2026-05-19.
			// Resolves U3 (SR/GPR at PC=ac008300) and U2 native-side (RAM bytes
			// at three guest PCs). Single INFO_LOG line — easy to grep.
			{
				const u8* ram_base = &mem_b[0];
				const u8* r1 = ram_base + 0x8300;
				const u8* r2 = ram_base + 0x83d8;
				const u8* r3 = ram_base + 0x2ab4c;
				...
				INFO_LOG(REIOS,
					...
					Sh4cntx.r[8], Sh4cntx.r[9], Sh4cntx.r[10], Sh4cntx.r[11],
					Sh4cntx.r[12], Sh4cntx.r[13], Sh4cntx.r[14], Sh4cntx.r[15],
					Sh4cntx.sr.getFull(), Sh4cntx.pr, Sh4cntx.gbr, Sh4cntx.vbr,
					Sh4cntx.mac.h, Sh4cntx.mac.l, Sh4cntx.fpscr.full,
					h1, h2, h3);
			}
		}
		else {
