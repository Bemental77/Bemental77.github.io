...
int maple_schid;
// DIAG (ORDER 21b maple-storm): repurposed as bit12 raise/clear counters
// (see holly_intc.cpp). g_dbg_maple_cyc = # asic raises of holly_MAPLE_DMA,
// g_dbg_maple_last = # W1C clears of bit12 via Write_SB_ISTNRM. Strip after verdict.
u32 g_dbg_maple_cyc = 0, g_dbg_maple_last = 0;
u32 g_dbg_istnrm_wr = 0, g_dbg_istnrm_wdata = 0;  // DIAG all SB_ISTNRM writes + last data
u32 g_dbg_last_intevt = 0;  // DIAG last INTEVT delivered (referenced by sh4_interrupts.cpp)
u32 g_dbg_maple_dodma = 0, g_dbg_maple_schd = 0;  // DIAG maple_DoDma vs maple_schd(completion) counts
u32 g_dbg_schd_armed0 = 0, g_dbg_schd_armed1 = 0;  // DIAG armed-flag@0x8c578988 value when completion fires
u32 g_dbg_int320 = 0, g_dbg_int360 = 0, g_dbg_intoth = 0;  // DIAG Do_Interrupt count per INTEVT
...
static void maple_DoDma()
{
	{ extern u32 g_dbg_maple_dodma; g_dbg_maple_dodma++; }  // DIAG DMA-start count
	verify(SB_MDEN & 1);
	verify(SB_MDST & 1);
...
static int maple_schd(int tag, int cycles, int jitter, void *arg)
{
	{ extern u32 g_dbg_maple_schd; g_dbg_maple_schd++; }  // DIAG completion count
	// DIAG: sample the guest "DMA-armed" flag (0x8c578988) at completion time.
	{ extern u32 g_dbg_schd_armed0, g_dbg_schd_armed1;
	  u32* ap = (u32*)GetMemPtr(0x8c578988u, 4);
	  if (ap && *ap == 0) g_dbg_schd_armed0++; else if (ap) g_dbg_schd_armed1++; }
	if (SB_MDEN & 1)
	{
