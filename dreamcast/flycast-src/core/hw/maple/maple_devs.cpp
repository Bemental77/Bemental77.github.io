...
// VMU bridge (wasm build). The emscripten VMU is in-memory only (see the
...
#ifdef __EMSCRIPTEN__
		// Headless WASM build: host VMU storage (hostfs::getVmuPath() /
		// nowide::fopen persistence) is not wired up. Create an in-memory-only
		// VMU: keep file == nullptr (fullSave() then no-ops) and format a blank
		// card from vmu_default so the guest sees a valid, empty memory card.
		// Session-only; IndexedDB persistence is a separate follow-up.
		file = nullptr;
		initializeVmu();
		fullSaveNeeded = false;
