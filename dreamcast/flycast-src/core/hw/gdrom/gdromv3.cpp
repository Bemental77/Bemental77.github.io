...
static void GDROM_DmaStart(u32 addr, u32 data)
{
	...
	if (SB_GDST == 1)
	{
		...
		// Fix B — redream-style synchronous DMA drain. Under our wasm JIT's
		// reduced throughput, sh4_sched_request may delay the GDROM completion
		// event past the BIOS/IPL polling timeout, leaving SB_GDST=1 and the
		// SB_ISTNRM bit 14 raise stalled. Redream (inolen/redream
		// src/guest/holly/holly.c:38-83) runs the equivalent of GDRomschd
		// inline inside the SB_GDST=1 write callback. GDRomschd's body above
		// already implements the synchronous redream shape (read sectors,
		// WriteMemBlock_nommu_ptr, asic_RaiseInterrupt at gdromv3.cpp:1295) —
		// we just need to call it inline until SB_GDST clears.
		while (SB_GDST & 1)
		{
			int t = GDRomschd(0, 0, 0, nullptr);
			if (t == 0) break;  // defensive: nothing more to drain
		}
	}
}
