...
u32 GetRTC_now()
{
	...
	//
	// [2026-09-08] PINNED UNCONDITIONALLY FOR LOCKSTEP. This is the host
	// wall clock reaching guest state, and it is the LAST of three sites
	// that did so from this one function:
	//   1. nvmem.cpp:218  srand(now) -> the Dreamcast console ID
	//   2. nvmem.cpp:204  syscfg.time_lo/hi -> written into flash
	//   3. aica_if.cpp:200 initRtc() -> RealTimeClock, read by the guest
	//      through the AICA RTC registers (readRtcReg, :56-70)
	// Sites 1 and 2 were pinned at the consumer in nvmem.cpp; site 3 could
	// only be fixed here. MEASURED, two instances cold-booting 3 s apart with
	// both at guest_cycles=0 and neither having run a guest instruction: the
	// power-on states differed in exactly one byte, @0x2003d5, which reads as
	// a little-endian u32 = 2,420,031,629 vs 2,420,031,632 — the 3-second gap
	// between the two boots, i.e. this clock.
	//
	// Upstream already does exactly this for its own netplay path (the
	// GGPOEnable branch this replaces) and the comment above is upstream's.
	// We take it unconditionally because lockstep IS the product here and
	// peers boot from frame 0 with no savestate, so there is no configuration
	// in which a host-seeded RTC is safe.
	//
	// NOT a continuous leak, so pinning once at init is sufficient: this is
	// read ONCE by initRtc(), and DreamcastSecond (:186) thereafter advances
	// RealTimeClock on the GUEST clock only; RealTimeClock is serialized
	// (:542, :584).
	//
	// COST, stated rather than buried: the console's date reads 1/1/70 plus
	// session uptime instead of the host's wall date. Elapsed-time logic
	// inside a session is UNAFFECTED, because the counter still advances once
	// per guest second — only the absolute date is fixed. A game that
	// displays a date shows 1970.
	// 1/1/70 00:00:00. Supersedes the GGPOEnable-only branch this replaces:
	// unconditional, so it holds on the lockstep path too (GGPO is not used
	// by this build).
