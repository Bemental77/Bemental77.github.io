...
static void add_isp_to_nvmem(DCFlashChip *flash, const char *username)
{
	...
	flash_bba_block bba{};
	if (!flash->ReadBlock(FLASH_PT_USER, FLASH_USER_BBA, &bba)
			|| memcmp(bba.sega, "SEGA", 4)
			|| bba.netmask == 0)
	{
		memcpy(bba.sega, "SEGA", 4);
		bba.netmask = 0xffffff00;
		// Leave everything else at 0 to use DHCP
...
static void fixUpDCFlash()
{
	if (settings.platform.isConsole())
	{
		...
		// [2026-09-08] `now` reaches GUEST STATE twice from here — syscfg.time_lo/hi
		// just below, and srand(now) for the console ID at :218 — and our flash is
		// IN-MEMORY ONLY, so it is blank on every boot and this path re-runs every
		// time. Two peers booting seconds apart got different console IDs before
		// either had executed a guest instruction.
		// This is now pinned AT THE SOURCE, in aica::GetRTC_now() (aica_if.cpp),
		// which is the single place all three host-clock sites read from. It is
		// deliberately NOT re-pinned here: three independent pins can drift apart.
