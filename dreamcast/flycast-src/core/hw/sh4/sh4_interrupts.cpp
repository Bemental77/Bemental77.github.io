...
// dreamcastHtml lever-5B: expose the TU-static interrupt-decode state's
// addresses so the SH4 emitter can inline the common-case shop_sync_sr
// (no bank change): decoded_srimask = BL ? 0 : ~InterruptLevelBit[IMASK];
// interrupt_pend = vpend & vmask & decoded. The game's imask set/restore
// primitives fire this ~400K/s in gameplay-class code; the C round trip
// per fire was a dominant cost. See dreamcast/docs/lever-5-heavy-phase.
extern "C" void sh4_intr_state_ptrs(u32* out4)
{
	out4[0] = (u32)(uintptr_t)&interrupt_vpend;
	out4[1] = (u32)(uintptr_t)&interrupt_vmask;
	out4[2] = (u32)(uintptr_t)&decoded_srimask;
	out4[3] = (u32)(uintptr_t)&InterruptLevelBit[0];
}
#endif
...
{
	CCN_INTEVT = intEvn;
	{ extern u32 g_dbg_last_intevt; g_dbg_last_intevt = (u32)intEvn; }  // DIAG storm ID
	{ extern u32 g_dbg_int320, g_dbg_int360, g_dbg_intoth;  // DIAG per-INTEVT delivery counts
	  if ((u32)intEvn == 0x320) g_dbg_int320++; else if ((u32)intEvn == 0x360) g_dbg_int360++; else g_dbg_intoth++; }
