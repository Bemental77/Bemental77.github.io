...
void Sh4Interpreter::Run()
{
	...
	// One-shot per-instruction trace (parity test 2026-05-19).
	// Logs the first 5000 SH4 instruction PCs to confirm whether native's
	// execution traversal matches our wasm's dense pctrace. Resolves U1.
	static int pc_trace_count = 0;
	static const int PC_TRACE_MAX = 5000;
			...
			try {
				do
				{
					if (pc_trace_count < PC_TRACE_MAX) {
						INFO_LOG(INTERPRETER, "[pc-trace] %d pc=%08x",
							pc_trace_count, ctx->pc);
						pc_trace_count++;
					}
					u32 op = ReadNexOp();
