...
// ===========================================================================
// [bench-block] Per-execution ns timer for ONE target SH4 block.
// ---------------------------------------------------------------------------
// At bm_AddBlock time, if the compiled block's vaddr matches BENCH_TARGET, we
// hijack the FPCA entry: instead of pointing to the JIT'd code, FPCA points to
// bench_shim() (a C function with the same void(*)() signature as a compiled
// block). bench_shim() reads __rdtsc, calls the real block, reads __rdtsc
// again, accumulates the delta. Every 0x10000 executions, INFO_LOGs:
//     [bench-block] addr=0xXXXX execs=N avg_cycles=M min_cycles=P
//
// rdtsc cycles are NOT converted to ns here (cycle frequency is variable on
// turbo-boosted i9; raw cycles per execution is the comparable number against
// our wasm 9044 ns/dispatch since both will be on the same host clock domain
// once we read /proc/cpuinfo equivalent). For now, report raw cycles.
// ---------------------------------------------------------------------------
#if defined(__x86_64__)
#include <mach/mach_time.h>
static constexpr u32 BENCH_TARGET = 0x8c02ab4cu;
typedef void (DYNACALL *BenchBlockFn)();
static std::atomic<BenchBlockFn> s_bench_real_fn{nullptr};
static std::atomic<uint64_t> s_bench_sum_abs{0};
static std::atomic<uint64_t> s_bench_count{0};
static std::atomic<uint64_t> s_bench_min_abs{UINT64_MAX};
static mach_timebase_info_data_t s_timebase = {0, 0};
static void DYNACALL bench_shim()
{
	BenchBlockFn fn = s_bench_real_fn.load(std::memory_order_acquire);
	if (!fn) return;  // not yet primed, skip dispatch
	// Volatile prevents the compiler from putting t0 in a caller-saved register
	// that the JIT'd block (fn()) might clobber across the call. Force t0 to a
	// callee-saved stack slot. Memory barriers prevent reordering.
	volatile uint64_t t0_v;
	volatile uint64_t t1_v;
	__asm__ __volatile__("" ::: "memory");
	t0_v = mach_absolute_time();
	__asm__ __volatile__("" ::: "memory");
	fn();
	__asm__ __volatile__("" ::: "memory");
	t1_v = mach_absolute_time();
	__asm__ __volatile__("" ::: "memory");
	const uint64_t t0 = t0_v;
	const uint64_t t1 = t1_v;
	if (t1 < t0) return;  // clock went backward — skip this sample (paranoia)
	const uint64_t delta = t1 - t0;
	const uint64_t c = s_bench_count.fetch_add(1, std::memory_order_relaxed) + 1;
	s_bench_sum_abs.fetch_add(delta, std::memory_order_relaxed);
	uint64_t cur_min = s_bench_min_abs.load(std::memory_order_relaxed);
	while (delta < cur_min && !s_bench_min_abs.compare_exchange_weak(cur_min, delta));
	if ((c & 0x3FFu) == 0) {  // every 1024 executions (lowered from 65536 for early visibility)
		// Convert mach_absolute_time units to ns via cached timebase.
		const uint64_t sum = s_bench_sum_abs.load(std::memory_order_relaxed);
		const uint64_t mn  = s_bench_min_abs.load(std::memory_order_relaxed);
		const uint64_t avg_abs = sum / c;
		const uint64_t avg_ns = (s_timebase.denom > 0) ? avg_abs * s_timebase.numer / s_timebase.denom : avg_abs;
		const uint64_t min_ns = (s_timebase.denom > 0) ? mn      * s_timebase.numer / s_timebase.denom : mn;
		INFO_LOG(DYNAREC, "[bench-block] addr=0x%08x execs=%llu avg_ns=%llu min_ns=%llu",
		         BENCH_TARGET,
...
void bm_AddBlock(RuntimeBlockInfo* blk)
{
	...
	// [bench-block] reverted — C++ shim crashes RetroArch with SIGILL on the
	// JIT'd-block calling boundary. Per-block ns measurement requires deeper
	// codegen instrumentation than this hook supports.
