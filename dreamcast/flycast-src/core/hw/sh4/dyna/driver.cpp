...
static DynarecCodeEntryPtr compilePC(u32 blockcheck_failures)
{
	...
	// [diagnostic] dump host x86 bytes for the wedge block at 0x8c02ab4c so we can
	// diff against the WASM-side blockdump in dreamcast/flycast-bridge/rec_wasm.cpp:735.
	if (rbi->vaddr == 0x8c02ab4cu) {
		std::string hex;
		const u8* p = (const u8*)rbi->code;
		hex.reserve(rbi->host_code_size * 2);
		const char* lut = "0123456789abcdef";
		for (u32 i = 0; i < rbi->host_code_size; ++i) {
			hex.push_back(lut[p[i] >> 4]);
			hex.push_back(lut[p[i] & 0xf]);
		}
		INFO_LOG(DYNAREC, "[host-bytes] vaddr=0x%08x host_code_size=%u oplist=%u guest_cycles=%u hex=%s",
		         rbi->vaddr, rbi->host_code_size, (u32)rbi->oplist.size(), (u32)rbi->guest_cycles, hex.c_str());
	}
...
{
	INFO_LOG(DYNAREC, "rdv_FailedToFindBlock %08x", pc);
	Sh4cntx.pc=pc;
