...
void sh4_sched_tick(int cycles)
{
	...
	if (sh4_sched_next_id != -1)
	{
		for (sched_list& sched : sch_list)
		{
			// Generalize the 2026-05-16 SPG VBLANK_IN scheduler-bucket fix
			// (see dreamcast/flycast-bridge/rec_wasm.cpp around the SPG
			// force-raise comment). Under our wasm JIT's reduced throughput,
			// guest-side tick gaps can exceed `cycles` between calls — events
			// whose `end` is now past `fztime` (remaining < 0) were being
			// silently dropped by the original `remaining >= 0 && remaining <= cycles`
			// gate. Fire them too: they're overdue, not invalid. Native rec
			// avoids this because its dispatch is fast enough that gaps never
			// exceed one timeslice. handle_cb's jitter computation already
			// accepts past-due events; the periodic callbacks (VBLANK, TMU,
			// AICA-DMA, MAPLE) just reschedule sooner to catch up.
			if (sched.end == -1)            // unscheduled — sh4_sched_remaining returns u32(-1)
				continue;
			int remaining = (int)(sched.end - fztime);
			if (remaining <= (int)cycles)
				handle_cb(sched);
		}
	}
	sh4_sched_ffts();
