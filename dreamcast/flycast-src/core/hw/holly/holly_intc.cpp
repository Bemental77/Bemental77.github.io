...
void asic_RaiseInterrupt(HollyInterruptID inter)
{
	...
	case 0:
		SB_ISTNRM |= mask;
		if (mask & 0x1000) { extern u32 g_dbg_maple_cyc; g_dbg_maple_cyc++; }  // DIAG bit12 raise
		break;
	case 1:
		SB_ISTEXT |= mask;
		break;
...
static void Write_SB_ISTNRM(u32 addr, u32 data)
{
	...
	if (Naomi2 && (addr & 0x02000000) != 0)
		...
		SB_ISTNRM &= ~data;
	if (data & 0x1000) { extern u32 g_dbg_maple_last; g_dbg_maple_last++; }  // DIAG bit12 W1C clear
	{ extern u32 g_dbg_istnrm_wr, g_dbg_istnrm_wdata; g_dbg_istnrm_wr++; g_dbg_istnrm_wdata = data; }  // DIAG all writes
