...
// code used to be enough to receive dreamcast.html's live video and audio, push
// player-2 input into port 1 and take the end-of-session save. lib/netplay.js
// now holds an inbound peer with none of that until a human allows it, and
// draws the dialog itself, so dreamcast.html needed no change to get it.
...
prompt && prompt.allow
  ? ok('host-is-asked-before-anything-flows', `Allow/Deny raised on dreamcast.html with confirmation code ${prompt.sas}`)
