...
2. **Before serving any HTML page locally**: use `npm run web` → `http://localhost:8080/<page>.html`. **`npm run web` IS `node tools/devserver.mjs`, NOT `python3 -m http.server`** (package.json:8, committed as `e362deb1`) — corrected 2026-09-08. The distinction is load-bearing, not cosmetic: python's server IGNORES HTTP Range and answers 200 with the whole file, so on it the streaming-disc path silently falls back to eager and every local run 'passes' without ever exercising the feature. `web:simple` (package.json:11) is still the python one; do not reach for it when Range matters. Do not improvise the port, do not invent a script, do not `npx serve` unless the existing one is dead. Check `package.json` "scripts" first.
...
10. **Measurement discipline — these were each paid for with a wrong claim.**
   ...
   - **The GameCube `[mips]` meter was MISUSED, not broken — corrected 2026-09-04.** Its `CREDITED` value **IS** the guest clock and matched the AI-DMA witnesses everywhere they were compared. The number that disagreed with MP4's own `GlobalCounter` (guest `0x801D3A54`, `~/gc_refs/marioparty4/src/game/main.c:115`) by ~1.7x is **`EXECUTED/486`**. The meter reports two different quantities and the wrong one was being quoted, including in this file. Quote `CREDITED` as the guest clock; treat `EXECUTED` as executed-throughput, which is a different question. (Historical: it produced 98.2/94.5/75.2/97.8% across runs under the old reading.) **It also SHIPS OFF as of 2026-09-02** and must be armed with **`?bjit_mips=1`**: its 6-op read-modify-write of `0x026B3420` in every block prologue measured 2.2pp of all executed emitted ops — 30% of the entire prologue — so it is now emit-time gated on SAB cell `0x026B39B8` (`ppc_emit.cpp` `bem_mips_census_on`). An unarmed run prints `[mips] METER OFF` and `exec=off`, which is **not** a zero-throughput reading; blocks must be compiled AFTER the cell is set, so arm it from the query string. Dreamcast has a real witness (AICA emits one frame per `AICA_TICK`=4535 SH4 cycles at 200 MHz, so production rate ÷ 44101.43 IS the guest clock → measured 0.999x); the GC analogue is the AI DMA callback at exactly 200 fires per unit time (`SystemTimers.cpp:80-84`).
   - **THE GC GUEST-RATE RIG NOW EXISTS: `gamecube/tools/guest_rate_witness.mjs`** (doc `gamecube/docs/guest-rate-witness/TASKS.md`). Hardware constant derived FROM DOLPHIN SOURCE, not from a doc: `486e6 × 3372 / 13.5e6 = 121,392` ticks → `4003.5587` callbacks/s. ⚠ `DSP.cpp:561`'s comment uses **3375, which is the Wii divisor** — GC is 3372.
   - ⚠ **`ai_dma_cb`, `aid_fire` and the third witness are THREE CODE PATHS ONTO ONE CLOCK — NOT independent corroboration.** They agreed to ≤0.0009 across 12 cells, which proves nothing about correctness. The genuinely independent checks are W4 (a guest-EXECUTED counter: MP4's `retraceCount`, agreeing to 0.10-0.16%; SAB's is at `0x803ADDC0`, and SAB's on-screen mission timer is a second, format MM:SS:CC — it read 0.426x against CoreTiming's 0.418x) and W5 (`drawn/s ÷ guest-rate`, recovering 58.3-59.9 fps on MP4/SAB and 29.7-30.0 on PSO). **`published/s` and `drawn/s` are the separate metrics; only `drawn/s` proves liveness.**
   - ⚠ **ALWAYS REPORT THE IDLE-SKIP FRACTION WITH A RATE.** `pso-cold` reads **0.9992x — a genuine 1.00x, W5-confirmed at full 30 fps — while 71.8% of credited time was clock-jumped and the JIT executed 137.3 MHz of a 486 MHz Gekko (0.2825x)**. Executed-vs-Gekko across scenes: 0.233x-0.407x; idle-skip 24.0%-71.8%. A rate without its idle fraction is not a result.
   - ⚠ **A `drawn/s` CELL IS NOT NECESSARILY SINGLE-VALUED.** SAB City Escape read 2.6 drawn/s in the truth table and **23 / 22.6** on two re-runs of the identical cell (same frozen wasm `82bc8f8b…`, same savestate, same query, restore proven). Do not treat one run as the value. Related: the end-of-run `canvas:` line is not evidence on its own — `sab-cold-mips` reported `nonBlack: 0` on a run whose screenshot is a full 3D scene.
   - **FALSE ZEROS the rig now refuses to print:** MP4 routes to the RECOMP engine with NO query parameter (`gamecube.html:1044`), so 26/26 windows read 0 callbacks and 0 credited cycles **while rendering 60 published/s** — that is not 0.000x. W4's addresses are MP4 symbols and read a constant on SAB/PSO. Boot-phase W4 read **862,252,765 retraces/s** off unlaid-out MEM1.

11. **THE PROBE MEASURES WHAT SHIPS — `--no-liftoff` was removed as the default 2026-09-04 (`d7c3415b`).** `gamecube/tools/dolphin_render_probe.js:159` used to default its V8 flags to `--no-liftoff`, forcing every wasm module straight into TurboFan. **A WEB PAGE CANNOT SET V8 FLAGS** — a visitor gets stock V8 (Liftoff first, TurboFan only after a module executes `--wasm-tiering-budget 13,000,000 / code_body_bytes` times). So every ABSOLUTE guest rate published before that commit described a configuration the product cannot be in; matched-pair RATIOS with both arms on the same flag are unaffected. `PROBE_JS_FLAGS=--no-liftoff` restores the old arm deliberately.
   - Measured in `gamecube/docs/wasm-tier/TASKS.md` (`6c210459`): the tier is worth **2.108x** on emitted bodies (318 blocks paired, per-arm floor subtracted, interleaved), but under stock V8 **260/323 modules tier up unaided**, so the residual the flag was handing us is **~1.164x**. ⚠ `webAssemblyFindings.md:525`'s "Verified" claim that there is *no* TurboFan tier-up for JIT'd blocks was FALSE — its evidence was blog posts, and a real emitted block reports `compiler: TurboFan`. A small body does not pin code in Liftoff; it RAISES the execution count needed to leave it.

12. **☠ THE OLD SR-vs-JIT PERFORMANCE NUMBERS ARE VOID (`9c18648e`) — do not quote `0.50-0.54x`, `0.62x`, `0.676x`, `0.4450x`, or `1.69x`.** The single fixture behind `0.50-0.54x` (`HandleReverb`) FAILS ITS OWN DIFFERENTIAL — 308 spurious `0xff` writes to a page native only reads — and `perf_browser.mjs` builds its restore set from `fx.writes` only, so rep 1 of ~160,000 timed invocations is the only one computing native's answer. `0.4450x` re-reads as **0.3338 / 0.3592 / 0.5010 / 0.5097** off ONE frozen binary. Structural cause: **the `-O2` perf build cannot host the verify harness at all**, so speed and correctness have never been measured on the same binary. **The honest JIT figure is 0.3781x delivered / 141.6 MHz executed / 23.2% idle-skipped on SAB cold-boot attract — 2.64x short of 1.000x** (1.000x there costs 486 × (1−0.232) = 373.5 MHz).
...
- **Local dev server**: `npm run web` (= `node tools/devserver.mjs`, port 8080) → http://localhost:8080/<page>.html. It implements HTTP Range/206, which `python3 -m http.server` does not — that gap is why the Range-streaming disc path went untested locally for so long. The site's `coi-serviceworker.js` installs COOP/COEP on first reload for SharedArrayBuffer.
...
`lib/capability.js` is loaded by **five** emulator pages — `gamecube.html`, `dreamcast.html`, `n64/index.html`, and (verified 2026-09-05 against the live files) **`ps1.html:4` and `snes.html:4`**, both of which also already call `Capability.autoGate(Capability.SPECS.ps1|snes)` (`ps1.html:788`, `snes.html:620`) against specs that already existed at `lib/capability.js:982-984`. ~~loaded by all three emulator pages~~ was STALE and cost a wrong plan: it made adding ps1/snes to `tools/device_matrix.mjs` look like a capability-layer integration when it needed **two `PAGES` rows and two `invariantFor` judges**. It publishes **`window.__cap`** — one report shape, so one rig can ask every page the same questions. **Every field is the result of DOING the thing, never of reading a property.** Measured 2026-09-01, Chrome 140: under `--disable-gpu` and under `--disable-features=WebGPUService,Dawn`, `navigator.gpu` is still **truthy** while `requestAdapter()` resolves **null** — and `--disable-features=WebGPU` / `--disable-blink-features=WebGPU` are **no-ops** that disable nothing. A presence check is not a capability check.
...
- **`dreamcast/flycast-src/` IS NO LONGER IGNORED WHOLESALE, AND THERE IS A DRIFT GATE.** It used to be ignored in its entirety (`git ls-files dreamcast/flycast-src` = 0 of 23,628 files), and on 2026-09-07 eight ported files SILENTLY REVERTED to pristine upstream Flycast — un-bisectable, because the broken tree was not in git; recovery needed a sibling checkout on another disk. Two of the lost edits killed the boot (`config::ThreadedRendering.override(false)` in `core/emulator.cpp`; the wasm guard on the read-only const-fold in `core/hw/sh4/dyna/ssa.cpp`) and one compiled out the ENTIRE ARM7 dynarec (`FEAT_AREC DYNAREC_JIT` in `core/build.h`). Now: the **34 files that differ from upstream `4be8a48` are tracked in place** via a `!`-negation block in `.gitignore` (so a reversion is an ordinary `git diff`), and **`dreamcast/tools/verify_core_tree.sh`** compares blob hashes against the nested checkout's upstream commit and runs automatically from `build_and_probe.sh` and `flycast_worker_link.sh` — a drifted tree cannot produce a binary. `--restore` recovers, `--sync-gitignore` records a new ported file, `DC_CORE_AUDIT=off` skips behind a loud banner. ⚠ **Do NOT verify this tree with `patch --dry-run`** — it lied in BOTH directions here (BSD `patch` skips already-applied hunks and exits 0; fuzz matched a `FEAT_AREC` hunk against the wrong one of `build.h`'s four identical `#define FEAT_AREC DYNAREC_NONE` lines) — and `dreamcast/flycast-bridge/patches/` is INCOMPLETE anyway: it covers 24 of the 33 flycast files that differ from upstream, so nine flycast files plus the one inside the `DreamPicoPort-API` submodule — ten in all — had no patch at all. Write-up: `dreamcast/docs/core-relink-broken/TASKS.md`.
