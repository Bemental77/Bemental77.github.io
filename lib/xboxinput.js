...
(function () {
  ...
  // ⚠ A CONNECTED GAMEPAD IS THE SIGNAL. A user-agent string is a GUESS.
  // Everything here used to be gated on onConsole(), which pattern-matches
  // navigator.userAgent for \bXbox\b etc. I have never seen a real Xbox Edge UA — my test
  // rig supplies a UA I invented that contains "Xbox", so every test passed BY
  // CONSTRUCTION. That is the same failure as the gamepadInputEmulation gate: the harness
  // creating the condition it is meant to detect. If the real UA does not match, the offer
  // is unreachable no matter what the visitor presses, which is exactly the reported
  // symptom ("I hit right trigger and I am not prompted").
  // A pad that is actually connected is not a guess, and it is the only condition that
  // really matters — if there is a controller, offering game controls is right on ANY
  // device, console or not.
  // ⚠ A NETPLAY GUEST IS NOT A CONTROLLER IN THIS ROOM.
  // n64/index.html presents a remote player to its core as a synthetic Gamepad
  // (that is the only way to reach the core's controller ports without rebuilding
  // the wasm — see its ONLINE CO-OP block). Everything that polls the Gamepad API
  // sees it, and this file's own poll then decided a controller had been plugged
  // in and put the full-screen "Use game controls?" card over the host's game.
  // MEASURED 2026-09-08: the card is what covered Mario Kart 64 the instant a
  // guest connected, and its own diagnostic panel named the culprit —
  // `controller: Bemental Netplay Remote Player`. The offer is meaningless for a
  // pad that is not in the room, so pads carrying that marker are skipped here and
  // in the console-detection poll below. A real controller is unaffected.
  function isRemotePad(pd) {
    return !!(pd && typeof pd.id === 'string' && pd.id.indexOf('Bemental Netplay') >= 0);
  }
  function anyPadConnected() {
    try {
      var pads = navigator.getGamepads ? navigator.getGamepads() : [];
      for (var i = 0; i < pads.length; i++) if (pads[i] && pads[i].connected && !isRemotePad(pads[i])) return true;
    } catch (e) {}
    return false;
  }
  function shouldOffer() { return HAS || onConsole() || anyPadConnected(); }

  function offerGameControls(why) {
    if (!shouldOffer()) return false;        // no pad, no console, no shell API: nothing to offer
  ...
  // answer it.
  try {
    if (shouldOffer() || true) {   // always arm: shouldOffer() is re-checked inside fire()
      var fired = false;
      var fire = function (why) {
        if (fired) return;
        // Re-check at FIRE time, not at arm time: a pad is usually connected LATER than
        // page load, so arming on the early answer would miss every controller.
        if (!shouldOffer()) return;
        fired = true;
        try { offerGameControls(why); } catch (e) {}
      };
      // ---- A CONTROLLER UPGRADES THE PAGE TO CONSOLE LAYOUT ----------------------
      // The `.console-ua` class is currently set only from a USER-AGENT MATCH, and that
      // test is duplicated TWELVE times across SEVEN files in two different regexes
      // (lib/xboxinput.js, gba/ps1/snes/dreamcast/n64/gamecube, plus a lowercase variant).
      // Every copy fails together if the real Xbox Edge UA does not contain "Xbox" — a
      // string I have never seen. When that happens the visitor gets the desktop layout on
      // a television AND no console handling at all, which is the reported experience.
      // A CONNECTED CONTROLLER IS A FACT. If one appears, treat this as a console for
      // layout purposes regardless of what the UA claims. This is additive: it can only
      // turn console handling ON for someone holding a pad, never off for anyone.
      // ⚠ It cannot retroactively change isMobile/render-path decisions already taken at
      // parse time — only the class-driven layout. That is the visible half.
      var markConsole = function (why) {
        try {
          var el = document.documentElement;
          if (el && !el.classList.contains('console-ua')) {
            el.classList.add('console-ua');
            if (typeof console !== 'undefined' && console.log) {
              console.log('[xboxinput] console-ua added by ' + why + ' (UA did not match)');
            }
          }
        } catch (e) {}
      };
      if (anyPadConnected()) markConsole('a pad already connected');

      // THE MISSING LISTENER. Chromium fires `gamepadconnected` on a pad's FIRST INPUT,
      // which is precisely the moment the owner pulls the right trigger. `grep -c
      // gamepadconnected` was 0 across this lib and every page, so that moment was
      // observed by nothing. It is also independent of the UA guess above.
      try {
        window.addEventListener('gamepadconnected', function (e) {
          markConsole('gamepadconnected');
          fire('gamepadconnected' + (e && e.gamepad ? ' id=' + String(e.gamepad.id).slice(0, 24) : ''));
        });
      } catch (e) {}
      ...
      var padWatch = function () {
        ...
        for (var i = 0; i < pads.length; i++) {
          ...
          if (isRemotePad(pd)) continue;          // a netplay guest, not a pad in this room
          markConsole('gamepad seen in poll');
