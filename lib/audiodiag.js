...
(function () {
  ...
  //   gba       null     — DELIBERATELY UNSET. The core is vendored with no
  //     source in the repo and `_emuSetSampleRate` (44gba.js) has NO CALL SITE.
  //     MEASURED 2026-09-01: it produces 47,909.7 frames/s into a 48,000 Hz
  //     context, i.e. it is already ~rate-matched and the absent
  //     _emuSetSampleRate call is NOT causing a mismatch — which is why nobody
  //     should "fix" that call blind. But 48,000 is the CONTEXT's rate, not a
  //     property of GBA hardware (the console's own mixer is nearer 32 kHz), so
  //     recording 48000 here would be dressing an implementation detail up as a
  //     hardware fact. Stays null until someone establishes the core's real
  //     target rate from source.
  var NOMINAL_RATE = {
    ps1: 44100,
    n64: 44100,
    gamecube: 32029,
  ...
  // A buffer that empties within this horizon is drifting, however small the
  // percentage. See the buffer-aware check in classify() for why a flat
  // tolerance alone reported HEALTHY on a page that was audibly gapping.
  var DRIFT_HORIZON_S = 600;
  ...
  function classify(o) {
    ...
    // BUFFER-AWARE DRIFT, checked BEFORE the flat tolerance.
    //
    // A fixed percentage tolerance is the wrong test and this was caught by a
    // real measurement, not by reasoning: GBA measured produced 47,909.7 f/s
    // against a 48,000 Hz sink = ratio 0.99543, which is INSIDE RATE_TOL, so
    // classify() returned HEALTHY — on a run that produced 6 audible gaps. What
    // actually matters is how long the buffer survives the imbalance, and with
    // AUDIO_FIFO_MAXLEN = 4900 frames (102 ms) a 0.46% deficit drains it in
    // 22.3 s. The same 0.46% against a multi-second buffer would be harmless.
    //
    // CALIBRATION NOTE, stated honestly: on that GBA run the mean-rate model
    // predicts ~1 underrun per 22.3 s (so 1 in the 34.3 s window) and 6 were
    // observed. Mean-rate drift is therefore a LOWER BOUND on underruns — real
    // producers are bursty and run dry earlier than the average implies. Do not
    // read secondsUntilEmpty as a forecast; read it as "no later than this".
    var fr = feedRatio(o.producedPerSec, o.consumedPerSec);
    if (fr !== null && fr < 1 && typeof o.capacityFrames === 'number' && o.capacityFrames > 0) {
      var tEmpty = secondsUntilEmpty(o.capacityFrames, o.producedPerSec, o.consumedPerSec);
      if (tEmpty !== null && tEmpty !== Infinity && tEmpty < DRIFT_HORIZON_S) {
        return r('DRIFT-STARVING',
          'feed ratio ' + fr.toFixed(5) + ' drains a ' + o.capacityFrames
            + '-frame buffer in ' + tEmpty.toFixed(1) + ' s (underruns no later than that, typically sooner)',
          'fix-the-producer-or-deepen-the-buffer');
      }
    }
  ...
  function report() {
    ...
    out.classification = classify({
      ...
      // Pass the real buffer depth so the buffer-aware drift check can fire.
      // Without it a sub-tolerance deficit reads HEALTHY however shallow the
      // buffer is — that is exactly how GBA's 0.46% deficit went unflagged.
      capacityFrames: d.capacity
    });
