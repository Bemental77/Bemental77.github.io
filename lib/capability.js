...
(function () {
  ...
  window.Capability = {
    ...
    SPECS: {
      // ⚠ 'degraded', and the history matters because this was tried, reverted, and is now
      // being redone on DIFFERENT evidence — do not read the revert as settling it.
      // FIRST ATTEMPT (reverted): demoting this while gamecube.html still killed Start on
      // its own policy only replaced an honest "Cannot run here — This device is missing
      // WebGPU." with a dead button reading "Start". That revert was right at the time.
      // WHAT CHANGED: the WebGL2 fallback now actually renders. A no-WebGPU device defaults
      // to the hardware path (it used to default to the software one, which measures a
      // static 4.5% non-black / 6 colours), gcRenderVerdict returns 'warn' for it with a
      // real explanation, and the measured result on that path is 29.4-42.4% non-black /
      // 167-179 colours at ~35 present fps with glErrors=0.
      // So 'required' now asserts something false — that the page cannot run here. It can,
      // reduced. 'degraded' routes through degraded()/verdict() to level 'warn', which
      // leaves Start usable (:805 disables only on 'bad') and tells the visitor quality is
      // reduced. That is exactly this case.
      // ⚠ Never 'ok' and never silent: guest work is still capped well below full speed by
      // the timer-query numbers, so the visitor must be told.
      gamecube:  { wasm: 'required', worker: 'required', coi: 'required', sab: 'required', webgpu: 'degraded' },
      ...
      gba:       { wasm: 'required' },
      snes:      { wasm: 'required' },
      // [genesis 2026-09-06] Same shape as snes for the same reasons, read off
      // what the page actually does: genesis/genesisWasm is a single-threaded
      // Emscripten link (genesis/genesisWasm/build.sh — no -pthread, no
      // -sSHARED_MEMORY), genesis.html loads no coi-serviceworker and creates
      // no Worker, and it presents through a 2D canvas context. So wasm is the
      // whole requirement; adding coi/sab/worker/webgl2 here would block
      // browsers that run the page perfectly.
      genesis:   { wasm: 'required' }
    },
