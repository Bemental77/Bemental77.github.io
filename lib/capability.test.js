...
(function () {
  ...
  function good() {
    return {
      ...
      webgl2: { ok: true, webgl1: true, fboComplete: true, stencil: true, depth: true,
                ...
                // WebGL2 ON AN OffscreenCanvas — a different capability from the
                // one above, measured on three surfaces. See REQ.webgl2Worker.
                offscreen: { supported: true, transferSupported: true, newOk: true, xferOk: true, ok: true, err: null },
                workerOffscreen: { tested: true, ok: true, stencil: true, depth: true, err: null } },
  ...
  var MUT = {
    ...
    // The Xbox/console/TV-browser shape: WebGL2 works on the main thread, and a
    // worker cannot have one. dreamcast.html renders ONLY from the worker, so
    // this must block it while leaving n64/ (main-thread renderer) alone.
    webgl2Worker: function (r) {
      r.webgl2.offscreen = { supported: true, transferSupported: true, newOk: false, xferOk: false, ok: false,
                             err: 'getContext("webgl2", {depth,stencil}) on an OffscreenCanvas returned null' };
      r.webgl2.workerOffscreen = { tested: true, ok: false, err: 'getContext("webgl2") returned null inside a worker' };
    },
  ...
  (function () {
    // ['webgl2','webgl2Worker'] and its converse are the pair MEASURED to be
    // independent on real hardware: under --disable-3d-apis a <canvas> refuses
    // WebGL2 while an OffscreenCanvas in a worker grants it, and under a
    // hypothetical console browser with no worker-GL the reverse holds. Folding
    // either into the other is the false negative that disabled Start on a
    // browser whose emulator was rendering.
    var independent = [['webgpu', 'webgl2'], ['webgpu', 'worker'], ['webgl2', 'sab'], ['worker', 'webgl2'],
                       ['webgl2', 'webgl2Worker'], ['webgl2Worker', 'webgl2']];
  ...
  // ── THE SECOND PRESENCE-vs-SURFACE CASE, and the one a real visitor hit ────
  // Same failure shape as `coi` above, on a different capability: a true
  // capability measured on a surface the page does not use.
  //
  // dreamcast.html transfers its canvas to the emulator worker
  // (dreamcast.html:2106) and renders WebGL2 only from there, but its spec said
  // `webgl2: 'required'` — the MAIN-THREAD probe. MEASURED 2026-09-04 under
  // Chrome --disable-3d-apis: a <canvas> returns null for webgl2 while an
  // OffscreenCanvas in a worker grants a full context with depth and stencil,
  // and the shipped page disabled Start reading "Cannot run here — This device
  // is missing WebGL2." while its worker printed drawingBuffer=640x480.
  //
  // Both directions are asserted. A guard that never blocks is as broken as one
  // that always does, and the console/TV case (main-thread WebGL2 fine, no
  // worker GL) is the one that must still stop the page.
  (function () {
    // (a) main-thread <canvas> refuses, the WORKER grants -> dreamcast RUNS.
    var r = good();
    r.webgl2.ok = false;
    r.webgl2.err = 'getContext("webgl2", {depth,stencil}) returned null';
    r.webgl2.fboComplete = null;
    C._setReport(r);
    var ids = C.blockers(SPECS.dreamcast).map(function (b) { return b.id; });
    ok('webgl2Worker/worker-context-overrides-main-thread-null', ids.indexOf('webgl2Worker') === -1,
       'the worker demonstrably got a WebGL2 context; dreamcast must NOT be blocked. blockers=' + ids.join(','));
    eq('webgl2Worker/main-thread-null-does-not-block-dreamcast', C.blockers(SPECS.dreamcast).length, 0);
    // ...and n64/, which renders on the MAIN thread, IS still blocked by it.
    eq('webgl2Worker/main-thread-null-still-blocks-n64', C.blockers(SPECS.n64).length, 1);

    // (b) the converse: main-thread WebGL2 fine, no context inside a worker.
    var r2 = good(); MUT.webgl2Worker(r2); C._setReport(r2);
    var ids2 = C.blockers(SPECS.dreamcast).map(function (b) { return b.id; });
    ok('webgl2Worker/blocks-when-only-the-worker-cannot', ids2.indexOf('webgl2Worker') >= 0,
       'a device with main-thread WebGL2 and no worker GL must block dreamcast. blockers=' + ids2.join(','));
    eq('webgl2Worker/does-not-block-n64', C.blockers(SPECS.n64).length, 0);

    // (c) the worker probe never ran (timeout / no Worker): fall back to the
    //     main-thread OffscreenCanvas spellings rather than reading silence as
    //     a refusal. `tested:false` must never be mistaken for `ok:false`.
    var r3 = good();
    r3.webgl2.workerOffscreen = { tested: false, ok: null, err: 'the worker probe did not settle' };
    C._setReport(r3);
    eq('webgl2Worker/untested-worker-falls-back-to-offscreen', C.blockers({ webgl2Worker: 'required' }).length, 0);
    var r4 = good();
    r4.webgl2.workerOffscreen = { tested: false, ok: null, err: 'the worker probe did not settle' };
    r4.webgl2.offscreen = { supported: false, transferSupported: false, newOk: null, xferOk: null, ok: false,
                            err: 'OffscreenCanvas is not defined' };
    C._setReport(r4);
    eq('webgl2Worker/untested-worker-and-no-offscreen-blocks', C.blockers({ webgl2Worker: 'required' }).length, 1);

    // (d) THE SAFARI-16.4 / no-offscreen-gl SHAPE: a worker on this device could
    //     take a WebGL2 context, but the PAGE has no transferControlToOffscreen
    //     to hand it one with. dreamcast.html:2154 has no other route, so the
    //     worker's yes is worthless and this must still block. A `has` that read
    //     only the worker's answer would pass it.
    var r5 = good();
    r5.webgl2.offscreen = { supported: true, transferSupported: false, newOk: true, xferOk: null,
                            ok: true, err: null };
    C._setReport(r5);
    var b5 = C.blockers({ webgl2Worker: 'required' });
    eq('webgl2Worker/no-transferControlToOffscreen-blocks', b5.length, 1);
    ok('webgl2Worker/no-transfer-why-names-the-handover',
       b5.length === 1 && /transferControlToOffscreen/.test(b5[0].why), b5.length ? b5[0].why : 'no blocker');

    // ...and with OffscreenCanvas absent outright, same answer, different reason.
    var r6 = good();
    r6.webgl2.offscreen = { supported: false, transferSupported: false, newOk: null, xferOk: null,
                            ok: false, err: 'OffscreenCanvas is not defined' };
    C._setReport(r6);
    var b6 = C.blockers({ webgl2Worker: 'required' });
    eq('webgl2Worker/no-offscreencanvas-blocks', b6.length, 1);
    ok('webgl2Worker/no-offscreencanvas-why-names-it',
       b6.length === 1 && /OffscreenCanvas/.test(b6[0].why), b6.length ? b6[0].why : 'no blocker');
  })();
