    ...
    return new Promise((resolve, reject) => {
      ...
      const start = () => {
        try {
          ...
          const byNonce = new Map();       // nonce -> the socket that nonce FIRST spoke on
          ...
          const wire = (c) => {
            ...
            c.on('data', (d) => {
              ...
              // a new one.
              if (d.n) {
                if (!byNonce.has(d.n)) byNonce.set(d.n, c);
                else if (byNonce.get(d.n) !== c) return;
              }
              if (cb) cb(d);
            });
            c.on('close', () => {
              conns.delete(c);
              const lost = [];
              for (const [k, v] of byNonce) if (v === c) { lost.push(k); byNonce.delete(k); }
    ...
    receive(m) {
      ...
      switch (m.t) {
        ...
        case 'lsbar': {          // the host's view of who the room is waiting on
          if (!this.isHost && m.b) this._emit('barrier', m.b);
          return true;
        }
  ...
  class NetplaySession {
    ...
    _linkGone(L, why) {
      ...
      if (!this._links.size) {
        ...
        if (this.isHost && !(this.ls && (this.ls.state === 'running' || this.ls.state === 'stalled'))) {
          this._status('signalling', 'waiting for someone to ask to join');
        } else {
          this._status('closed');
          this._emit('close');
    ...
    _bindChannel(dc, L) {
      ...
      dc.onclose = () => this._linkGone(L, 'channel-closed');
      dc.onmessage = (e) => {
    ...
    _ensureLockstep() {
      if (!this.ls) this.startLockstep(Object.assign({ portCount: this.portCount }, this._lsOpts || {}));
      return this.ls;
    }
