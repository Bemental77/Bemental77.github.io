        ...
        try {
          ...
          peer.on('error', (e) => {
            ...
            if (t === 'peer-unavailable' && !opts.host) {
              setTimeout(() => { try { wire(peer.connect('bemental-' + room + '-h', { reliable: true })); } catch (_) {} }, 1200);
              return;
            }
    ...
    async _onSignal(m) {
      ...
      try {
        if (m.t === 'hello' && this.isHost && pc.localDescription) {
          ...
          // ⚠ THE OFFER ARRIVES MORE THAN ONCE BY DESIGN: the host sends one at
          // start() and re-sends on 'hello' so a guest that was not listening yet
