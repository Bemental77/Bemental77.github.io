...
(function (global) {
  ...
  // ⚠ THE GAME CONNECTION HAD STUN AND NO TURN, WHILE THE SIGNALLING CHANNEL
  // UNDERNEATH IT HAD BOTH — so the two peers could always agree on how to
  // talk and then sometimes fail to actually talk. STUN only tells a peer its
  // own public address; it does not carry traffic, so two endpoints behind
  // NATs that will not hairpin (symmetric NAT, some carrier-grade NAT, some
  // corporate networks) discover each other and then have nowhere to send
  // anything. A relay is the only fix for that case.
  //
  // These are the servers peerjs 1.5.4 already ships and already uses for its
  // own DataConnection — verified by reading the bundle this page loads:
  //   $ grep -o 'iceServers:\[[^]]*\]' peerjs.min.js
  //   iceServers:[{urls:"stun:stun.l.google.com:19302"},
  //               {urls:["turn:eu-0.turn.peerjs.com:3478",
  //                      "turn:us-0.turn.peerjs.com:3478"], ...
  // So this adds no new third party and no new failure mode: the page cannot
  // pair at all without that library, and reusing its relay means the game
  // connection can go everywhere the signalling connection can. On a network
  // where a direct path exists, ICE prefers it and the relay is never used.
  const ICE_SERVERS = [
        ...
        try {
          ...
          const knock = () => {
            ...
            if (knocks > PEER_MAX_KNOCKS) {
              // ⚠ 3 above. This is the message that distinguishes a mistyped
              // code from a host who has not pressed Host yet — previously
              // both were an eternal "Looking for the host…".
          ...
          peer.on('error', (e) => {
            ...
            if (t === 'peer-unavailable' && !opts.host) { setTimeout(knock, PEER_KNOCK_MS); return; }
            clearTimeout(openTimer);
  ...
  class NetplaySession {
    ...
    async start() {
      ...
      const pc = this._pc = new RTCPeerConnection({ iceServers: ICE_SERVERS });
