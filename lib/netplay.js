...
(function (global) {
  ...
  class Lockstep {
    ...
    _scheduleDelay(next) {
      ...
      this._emit('delay', { from: this.delay, to: d, at: at, reason: 'the room stalled on a slow link' });
      return true;
    }
    _applyPendingDelay(f) {
      ...
      const was = this.delay;
      if (pd.d <= was) return;
      const last = this._lastLocal || {};
      for (let t = f + was + 1; t <= f + pd.d; t++) {
        const items = [];
        ...
        if (items.length) { this.stats.sent++; this._send({ t: 'ls', f: t, i: items, peer: this.peerId }); }
      }
      this.delay = pd.d;
    }
    ...
    beginFrame(pads) {
      ...
      if (this._scheduledTo <= f) {
        ...
        if (items.length) { this.stats.sent++; this._send({ t: 'ls', f: at, i: items, peer: this.peerId }); }
      }
      ...
      if (waitingOn.length) {
        ...
        if (!this._stalled) {
        ...
        } else if (this.isHost && this.delay < 30 &&
          ...
          //
          // It only ever goes UP. Lowering it while peers are already queueing
          // for the old target would drop input on the floor, and a delay that
          // oscillates is worse than one that is merely too large — the cost of
          // too large is input lag the player can feel; the cost of too small is
          // a room that does not run at all.
    ...
    endFrame(hash, words) {
      ...
      this.frame = f + 1;
      this.stats.frames++;
      if (hash != null) this.submitHash(f, hash, words);
      const cut = f - LS_KEEP;
      if (cut > 0 && (f % 64) === 0) {
  ...
  const RELAYED = { ls: 1, lsh: 1, lsd: 1, lsping: 1, lspong: 1 };
  class NetplaySession {
    constructor(opts) {
      ...
      this.relay = null;
      this._relayOut = [];          // coalesced 'ls' traffic awaiting a publish
      this._relayTimer = 0;
      this._relaySeq = 0;
    ...
    _link(nonce) {
      ...
      pc.onconnectionstatechange = () => {
        if (pc.connectionState === 'connected') this._disarmIce(L);
        if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected') this._linkGone(L, pc.connectionState);
      };
      if (this.isHost) this._bindChannel(pc.createDataChannel('play', { ordered: true }), L);
      else pc.ondatachannel = (e) => this._bindChannel(e.channel, L);
      return L;
    }
    ...
    _linkGone(L, why, keepSeat) {
      ...
      if (!this._links.size) {
        ...
        if (!this.isHost && !started && wasOpen && this._sig && this.state !== 'closed' && why !== 'left') {
          ...
          this._answered = false;
          this._chal = null;
          this._status('signalling', 'the link to the room dropped — asking to be let back in');
          this._sendHello();
          return;
        }
    ...
    _relaySend(o) {
      ...
      if (o && o.t === 'ls') {
        this._relayOut.push(o);
        if (!this._relayTimer) this._relayTimer = setTimeout(() => { this._relayTimer = 0; this._relayFlush(); }, RELAY_BATCH_MS);
        return;
      }
      this._relayFlush([o]);
    }
    ...
    _relayEmit(L, batch) {
      this._relayTx = (this._relayTx || Promise.resolve())
        .then(() => this._relayEmit1(L, batch)).catch(() => {});
    }
    async _relayEmit1(L, batch) {
      const seal = await this._relaySeal(L);
      if (!seal || !this._sig || L.dead) return;
      const s = subtle();
      const iv = randomBytes(12);
      const body = JSON.stringify({ q: ++this._relaySeq, b: batch });
      try {
    ...
    async _onRelay(m) {
      ...
      if (this.isHost) {
        ...
        if (!L || !L.approved || !L.relay) return;
      } else {
      ...
      try {
        ...
        obj = JSON.parse(new TextDecoder().decode(pt));
      } catch (e) { return; }              // not from the peer this link belongs to
      ...
      if (prev != null && !(obj.q > prev)) return;
      ...
      for (const one of obj.b) {
        if (one && one.t === 'rping') { this._relayEmit(L, [{ t: 'rpong', ts: one.ts }]); continue; }
    ...
    async _onSignal(m) {
      ...
      try {
        ...
        if (m.t === 'hello' && this.isHost) {
          ...
          if (this.ls && (this.ls.state === 'running' || this.ls.state === 'stalled')) {
            return this._sig.send({ t: 'denied', to: m.n,
              why: 'this room has already started — the controllers were plugged in when the disc loaded, so ask for a new code' });
          }
    ...
    _onRoomMsg(m, L) {
      ...
      if (this.isHost && RELAYED[m.t]) this._relay(m, L);
      return handled;
    }
    _relay(m, from) {
      const s = JSON.stringify(m);
      for (const L of this._links.values()) {
        if (L === from || !L.dc || L.dc.readyState !== 'open') continue;
        try { L.dc.send(s); } catch (e) {}
      }
    }
    ...
    _onPeerUp(L) {
      ...
      if (ls && !this.isHost) { try { ls.requestRoster(); } catch (e) {} }
      ...
      if (L && L.nonce !== HOST_LINK) ls.seat(L.nonce, 1);
      this._emit('room', this.roomInfo());
    }
    ...
    async rttReport(timeoutMs) {
      const ls = this.ls;
      const me = (ls && ls.peerId) || this._nonce;
      const peers = [];
      if (ls) for (const p of ls.roster) if (p != null && p !== me && peers.indexOf(p) < 0) peers.push(p);
      const out = {};
      for (const p of peers) out[p] = await this.pingPeer(p, timeoutMs);
      const got = Object.keys(out).map((k) => out[k]).filter((v) => v != null);
      return { peers: out, worstMs: got.length ? Math.max.apply(null, got) : null,
               recommendedDelay: got.length ? Lockstep.recommendDelay(Math.max.apply(null, got), 1000 / 60) : null };
    }
