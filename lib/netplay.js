...
(function (global) {
  ...
  // stated in one place that a reader can enumerate.
  const SIGNALS = {
  ...
  async function openSignal(want, room, opts) {
    return await (SIGNALS[want] || SIGNALS.local)(room, opts);
  }
    ...
    async _onSignal(m) {
      ...
      try {
        ...
        if (m.t === 'offer' && !this.isHost) {
          ...
          if (!L.pc.currentRemoteDescription) await L.pc.setRemoteDescription({ type: 'answer', sdp: m.sdp });
        } else if (m.t === 'ice') {
          if (this.isHost) {
