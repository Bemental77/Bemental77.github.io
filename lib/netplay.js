...
(function (global) {
  ...
  class Lockstep {
    ...
    constructor(opts) {
      ...
      this.delayCalmMs = opts.delayCalmMs == null ? 15000 : opts.delayCalmMs | 0;
    ...
    _scheduleDelay(next) {
      ...
      if (d < this.delay) {
        ...
        this._emit('delay', { from: this.delay, to: d, at: atDown,
                              reason: 'the link has been steady, so the room is giving the lag back' });
        return true;
      }
    ...
    // 1000 ms lag.
    _maybeGiveDelayBack() {
      if (!this.isHost || this._pendingDelay) return false;
      if (this.state !== 'running') return false;
      const floor = Math.max(2, this._delayFloor | 0);
      if (this.delay <= floor) return false;
      const now = this._now();
      const calm = this.delayCalmMs || 15000;
      ...
      return this._scheduleDelay(this.delay - 1);
    }
      ...
      switch (m.t) {
        case 'ls': {
          ...
          for (const pair of m.i) {
            ...
            try { this._put(m.f, port, b64dec(pair[1])); if (fresh) this.stats.received++; } catch (e) {}
          }
          return true;
        }
        case 'lsh': {
