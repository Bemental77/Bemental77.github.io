...
layout(set = 1, binding = 0) uniform texture2DArray samp_tex0;
layout(set = 1, binding = 1) uniform texture2DArray samp_tex1;
layout(set = 1, binding = 2) uniform texture2DArray samp_tex2;
layout(set = 1, binding = 3) uniform texture2DArray samp_tex3;
layout(set = 1, binding = 4) uniform texture2DArray samp_tex4;
layout(set = 1, binding = 5) uniform texture2DArray samp_tex5;
layout(set = 1, binding = 6) uniform texture2DArray samp_tex6;
layout(set = 1, binding = 7) uniform texture2DArray samp_tex7;
layout(set = 1, binding = 8) uniform sampler samp_smp0;
layout(set = 1, binding = 9) uniform sampler samp_smp1;
layout(set = 1, binding = 10) uniform sampler samp_smp2;
layout(set = 1, binding = 11) uniform sampler samp_smp3;
layout(set = 1, binding = 12) uniform sampler samp_smp4;
layout(set = 1, binding = 13) uniform sampler samp_smp5;
layout(set = 1, binding = 14) uniform sampler samp_smp6;
layout(set = 1, binding = 15) uniform sampler samp_smp7;
...
int4 sampleTexture_0(int2 uv, int layer) {
  float size_s = float(texdim[0u].x * 128);
  float size_t = float(texdim[0u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(0u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex0, samp_smp0), coords, lod_bias));
}
int4 sampleTexture_1(int2 uv, int layer) {
  float size_s = float(texdim[1u].x * 128);
  float size_t = float(texdim[1u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(1u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex1, samp_smp1), coords, lod_bias));
}
int4 sampleTexture_2(int2 uv, int layer) {
  float size_s = float(texdim[2u].x * 128);
  float size_t = float(texdim[2u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(2u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex2, samp_smp2), coords, lod_bias));
}
int4 sampleTexture_3(int2 uv, int layer) {
  float size_s = float(texdim[3u].x * 128);
  float size_t = float(texdim[3u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(3u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex3, samp_smp3), coords, lod_bias));
}
int4 sampleTexture_4(int2 uv, int layer) {
  float size_s = float(texdim[4u].x * 128);
  float size_t = float(texdim[4u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(4u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex4, samp_smp4), coords, lod_bias));
}
int4 sampleTexture_5(int2 uv, int layer) {
  float size_s = float(texdim[5u].x * 128);
  float size_t = float(texdim[5u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(5u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex5, samp_smp5), coords, lod_bias));
}
int4 sampleTexture_6(int2 uv, int layer) {
  float size_s = float(texdim[6u].x * 128);
  float size_t = float(texdim[6u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(6u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex6, samp_smp6), coords, lod_bias));
}
int4 sampleTexture_7(int2 uv, int layer) {
  float size_s = float(texdim[7u].x * 128);
  float size_t = float(texdim[7u].y * 128);
  float3 coords = float3(float(uv.x) / size_s, float(uv.y) / size_t, layer);
  uint texmode0 = samp_texmode0(7u);
  float lod_bias = float(bitfieldExtract(int(texmode0), 8, 16)) / 256.0f;
  return iround(255.0 * texture(sampler2DArray(samp_tex7, samp_smp7), coords, lod_bias));
}
...
VARYING_LOCATION(5)  in float3 tex3;
VARYING_LOCATION(6)  in float3 tex4;
VARYING_LOCATION(7)  in float3 tex5;
VARYING_LOCATION(8)  in float3 tex6;
VARYING_LOCATION(9)  in float3 tex7;
VARYING_LOCATION(10)  in float4 clipPos;
int2 selectTexCoord(uint index, int2 fixpoint_uv0, int2 fixpoint_uv1, int2 fixpoint_uv2, int2 fixpoint_uv3, int2 fixpoint_uv4, int2 fixpoint_uv5, int2 fixpoint_uv6, int2 fixpoint_uv7) {
  if (index >= 8u) {
    return fixpoint_uv0;
  }
  if (index < 4u) {
    if (index < 2u) {
      ...
      return (index == 2u) ? fixpoint_uv2 : fixpoint_uv3;
    }
  } else {
    if (index < 6u) {
      return (index == 4u) ? fixpoint_uv4 : fixpoint_uv5;
    } else {
      return (index == 6u) ? fixpoint_uv6 : fixpoint_uv7;
    }
  }
}
...
uint selectTexCoordIndex(uint texmap){
  ...
  if (texmap < 4u) {
    if (texmap < 2u) {
      return (texmap == 0u) ? 0u : 1u;
    } else {
      return (texmap == 2u) ? 2u : 3u;
    }
  } else {
    if (texmap < 6u) {
      return (texmap == 4u) ? 4u : 5u;
    } else {
      return (texmap == 6u) ? 6u : 7u;
    }
  }
}

int4 sampleTextureWrapper(uint sampler_num, int2 uv, int layer) {
...
#define getTexCoord(index) selectTexCoord((index), fixpoint_uv0, fixpoint_uv1, fixpoint_uv2, fixpoint_uv3, fixpoint_uv4, fixpoint_uv5, fixpoint_uv6, fixpoint_uv7)
...
void main()
{
  ...
  for(uint stage = 0u; stage <= num_stages; stage++)
  {
    ...
    int2 fixpoint_uv1 = int2((tex1.z == 0.0 ? tex1.xy : tex1.xy / tex1.z) * float2(texdim[1].zw * 128));
    int2 fixpoint_uv2 = int2((tex2.z == 0.0 ? tex2.xy : tex2.xy / tex2.z) * float2(texdim[2].zw * 128));
    int2 fixpoint_uv3 = int2((tex3.z == 0.0 ? tex3.xy : tex3.xy / tex3.z) * float2(texdim[3].zw * 128));
    int2 fixpoint_uv4 = int2((tex4.z == 0.0 ? tex4.xy : tex4.xy / tex4.z) * float2(texdim[4].zw * 128));
    int2 fixpoint_uv5 = int2((tex5.z == 0.0 ? tex5.xy : tex5.xy / tex5.z) * float2(texdim[5].zw * 128));
    int2 fixpoint_uv6 = int2((tex6.z == 0.0 ? tex6.xy : tex6.xy / tex6.z) * float2(texdim[6].zw * 128));
    int2 fixpoint_uv7 = int2((tex7.z == 0.0 ? tex7.xy : tex7.xy / tex7.z) * float2(texdim[7].zw * 128));
