...
using bemental::powerpc::emit_block_body;        // gekko per-block body
using bemental::powerpc::build_region_module;    // assemble fn_0..fn_N with internal funcref table
...
int main(int argc, char** argv) {
    ...
    bemental::powerpc::g_aot_chain = false;        // [aot-gpb] use the INTERNAL funcref table (self-
                                                   // contained) for intra-region return_call_indirect;
                                                   // no external env.aot_table dependency (fn_N modules).
    ...
    for (const auto& fn : funcs) {
        ...
        // [aot-gpb] GEKKO per-block: emit_block_body (with lookup -> intra-region return_call_indirect
        // via the internal funcref table = fast in-wasm chaining) instead of build_region_function.
        // Isolates whether the DLBuf overflow is in build_region_function's entry_sel/shared-locals
        // structure vs the per-block op emit (which conformance says is correct for integer ops).
        std::vector<uint8_t> bodies;
        bool body_fail = false;
        for (size_t i = 0; i < blocks.size(); i++) {
            std::vector<uint8_t> body = emit_block_body(
        ...
        std::vector<uint8_t> mod = build_region_module(bodies.data(), bodies.size(), (uint32_t)blocks.size());
