...
// Wasm target: netplay sources are gated off in Source/Core/Core/CMakeLists.txt
// (SFML is unsupported under emcc). This stub satisfies the references in
// upstream consumers (Core.cpp, BootManager.cpp, BTEmu.cpp, etc.) with no-ops.
// Authored fresh for the wasm build — not ported from any prior fork.
#include <span>
#include "Common/CommonTypes.h"
// NetPlayProto.h owns the canonical NetSettings + NetTraversalConfig types.
// It does NOT pull SFML transitively, so it's safe to include from this stub.
#include "Core/NetPlayProto.h"
namespace NetPlay {
bool IsNetPlayRunning();  // defined in gamecube/dolphin-bridge/dolphin_stubs.cpp
class NetPlayClient {
public:
...
inline unsigned int NetPlay_GetLocalWiimoteForSlot(unsigned int slot) { return slot; }
inline bool NetPlay_GetWiimoteData(const std::span<NetPlayClient::WiimoteDataBatchEntry>&) { return false; }
}  // namespace NetPlay
#else
