...
bool Wiimote::Read()
{
  ...
  {
#ifndef __EMSCRIPTEN__
    // Balance-board report broadcast via SFML UDP for external debug capture;
    // SFML is gated off under emcc, so this debug sink is skipped on wasm.
...
#ifndef __EMSCRIPTEN__
  // See balance-board broadcast comment above; same gate.
  if (m_balance_board_dump_port > 0 && m_index == WIIMOTE_BALANCE_BOARD)
  {
    static sf::UdpSocket Socket;
