                                             ...
                                             const Memcard::HeaderData& memcard_header_data)
{
  ...
  case EXIDeviceType::EthernetIPC:
    ...
    {
    case EXIDeviceType::Ethernet:
      result = std::make_unique<CEXIETHERNET>(system, BBADeviceType::TAP); break;
    case EXIDeviceType::EthernetTapServer:
      result = std::make_unique<CEXIETHERNET>(system, BBADeviceType::TAPSERVER); break;
    case EXIDeviceType::EthernetXLink:
      result = std::make_unique<CEXIETHERNET>(system, BBADeviceType::XLINK); break;
    case EXIDeviceType::EthernetBuiltIn:
      result = std::make_unique<CEXIETHERNET>(system, BBADeviceType::BuiltIn); break;
    case EXIDeviceType::EthernetIPC:
      result = std::make_unique<CEXIETHERNET>(system, BBADeviceType::IPC); break;
    default: break;
    }
#else
    // Broadband Adapter unavailable under emscripten (SFML / host networking
    // unsupported). Fall back to the default no-op EXI device.
    result = std::make_unique<IEXIDevice>(system);
#endif
    break;
...
#else
    // Gecko debug device uses SFML/Network; unsupported under emcc.
    result = std::make_unique<IEXIDevice>(system);
#endif
    break;
