...
#elif __EMSCRIPTEN__
// Upstream OS_* enum has no emcc bit; OS_ALL alone matches OS-agnostic bugs
// and skips host-specific driver workarounds (which don't apply in wasm).
...
static const char* to_string(Driver driver)
{
  switch (driver)
  {
    ...
    case DRIVER_V3D:         return "Broadcom";
