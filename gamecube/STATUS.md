...
- **BUG 2 (REAL, PINNED RED, fix deferred): non-branch-terminated blocks return the LAST op's address as next_pc.** Pre-op set_pc writes op.address; native non-branch emitters never advance PC; epilogue returns ctx PC → `JitWasm.cpp:235` (`ppc_state.pc = next_pc`) re-dispatches AT the already-executed boundary op → **boundary-op double-execution at every decode-cap cut** (non-idempotent cases: stwu/lwzu double rA-update, MMIO store double-fire). Legacy build_block returns the +4 fallthrough (test_gekko asserts it; suite green on legacy).
