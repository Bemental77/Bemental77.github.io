...
| 2 — Conformance harness | **IN PROGRESS — BUG 3 PINNED RED: lfs D-form EA** | `test_diff_next` 3457/0/452. `test_gekko_next`: 33 tests, **`lfs_sda2_negative_offset` RED** (exact HuSysInit retail bytes `C0228008 D02D86E0`; stored 0x0 vs 0x3F800000) — this IS the boot wedge root (STATUS.md 2026-06-11 FINAL: corrupted minimumVcount=-1 → HuSysDoneRender unsigned pacing loop never exits). Earlier: fastmem-guard wrap FIXED; boundary-PC double-exec FIXED (conditional epilogue v2). **NEXT ACTION: fix the FP-load EA path until lfs_sda2_negative_offset is green, full probe, expect cascade to first present.** Then: g_gfx SWGfx swap fix (video_cb), FP/PS coverage, RLWNM vectors, gate-#8 diag strip |
