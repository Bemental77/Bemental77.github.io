...
static void SOUND_FillAudio(void *unused, Uint8 *stream, int len) {
	...
	#ifdef __EMSCRIPTEN__
    EM_ASM_({if(typeof pcsx_worker!=='undefined'&&pcsx_worker)pcsx_worker.postMessage({cmd:"soundBytes", lBytes: $0});}, lBytes);
	#endif
...
void SoundFeedStreamData(unsigned char *pSound, long lBytes) {	
	...
	while (lBytes > 0) {
		if (((iWritePos + 1) % iBufSize) == iReadPos) {
			...
			#ifdef __EMSCRIPTEN__
			EM_ASM_({if(typeof pcsx_worker!=='undefined'&&pcsx_worker)pcsx_worker.postMessage({cmd:"soundBytes", lBytes: $0});}, old_lBytes-lBytes);
			#endif
			break;}
